...
We generate comprehensive test configurations with diverse characteristics to evaluate AILS across a wide range of scenarios:
\begin{itemize}
    \item \textbf{Grid sizes}: 50$\times$50, 100$\times$100, 150$\times$150, 200$\times$200, 250$\times$250, 300$\times$300, 400$\times$400, 500$\times$500
    \item \textbf{Obstacle densities}: 10\%, 15\%, 20\%, 25\%, 30\%, 35\%, 40\%
    \item \textbf{Patterns}: Random (uniform distribution), Clustered (grouped obstacles), Maze (corridors and walls), Room (structured spaces with doorways), Open (sparse obstacles)
    \item \textbf{Test pairs per configuration}: 50-200 random start-goal pairs with verified path existence
\end{itemize}

This systematic evaluation across 8 grid sizes, 7 density levels, and 5 patterns enables comprehensive analysis of AILS behavior across diverse environments.

\subsection{Baseline Algorithms}

We compare AILS against established pathfinding algorithms:
\begin{enumerate}
    \item \textbf{Standard A*}: Baseline optimal pathfinding with Manhattan heuristic for 4-connected grids and Euclidean heuristic for 8-connected grids
    \item \textbf{Dijkstra's Algorithm}: Uninformed optimal search serving as a performance baseline
    \item \textbf{Breadth-First Search (BFS)}: Unweighted optimal search for uniform-cost grids
    \item \textbf{Bidirectional A*}: Bidirectional search variant exploring from both start and goal
\end{enumerate}
...
\begin{itemize}
    \item AILS-Base achieves the lowest mean node count (1013), representing 56.9\% reduction vs A*
    \item AILS-Adaptive achieves 51.1\% node reduction with lower time variance
    \item All AILS variants maintain the same 99\% success rate as baseline algorithms
    \item Dijkstra and BFS explore 6.5$\times$ more nodes than A*, confirming the value of informed search
\end{itemize}
...
Key observations:
\begin{itemize}
    \item \textbf{Node reduction scales with grid size}: From 5.1\% at 50$\times$50 to 76.8\% at 500$\times$500
    \item \textbf{Time efficiency crossover}: AILS-Adaptive becomes time-competitive at 300$\times$300 and larger grids
    \item \textbf{Corridor overhead}: On small grids, AILS overhead exceeds search savings; on large grids, the substantial node reduction compensates for corridor construction time
\end{itemize}
...
Key findings:
\begin{itemize}
    \item \textbf{Optimal density range}: AILS performs best at 10-20\% density, achieving both time and node reduction
    \item \textbf{Transition point}: At 25\% density, AILS maintains strong node reduction but with time overhead
    \item \textbf{High density degradation}: Above 35\% density, corridor expansion significantly impacts performance
\end{itemize}
...
\textbf{Pattern-specific insights:}
\begin{itemize}
    \item \textbf{Random/Open patterns}: AILS achieves 43-58\% node reduction with competitive time
    \item \textbf{Clustered patterns}: High obstacle density in clusters causes corridor expansion overhead
    \item \textbf{Maze patterns}: Constrained paths along walls limit AILS's corridor advantage
    \item \textbf{Room patterns}: Very high density (90.9\%) but structured passages favor A*
\end{itemize}

These results indicate AILS is most effective for random and open environments with low-to-moderate obstacle densities, which are common in many robotics applications.
...
AILS achieves significant reductions in explored nodes:
\begin{itemize}
    \item \textbf{AILS-Base vs A*}: 56.0\% node reduction (Cohen's $d$ = 0.82, large effect)
    \item \textbf{AILS-Adaptive vs A*}: 51.4\% node reduction (Cohen's $d$ = 0.76, medium effect)
    \item \textbf{Statistical significance}: $p < 0.001$ for both comparisons
\end{itemize}

The node reduction demonstrates that AILS effectively restricts the search space while maintaining path quality. However, it is important to note that reduced node exploration does not always translate directly to reduced execution time, as corridor construction overhead must be considered.

\subsubsection{Time-Node Trade-off}
Our results reveal an important trade-off between time and node efficiency:
\begin{itemize}
    \item \textbf{Small grids (50-150)}: Corridor construction overhead exceeds search savings; A* is faster
    \item \textbf{Medium grids (200-250)}: AILS achieves substantial node reduction but with time overhead
    \item \textbf{Large grids (300+)}: Node reduction benefits outweigh overhead; AILS becomes time-competitive
\end{itemize}
...
Our pattern analysis reveals that AILS performance varies significantly with obstacle distribution:
\begin{itemize}
    \item \textbf{Random/Open patterns (best case)}: 43-58\% node reduction with competitive or better execution time. These environments have evenly distributed obstacles where local density estimation is most accurate.
    \item \textbf{Clustered patterns}: Substantial corridor expansion near clusters creates overhead. While node reduction is still achieved, time performance degrades significantly.
    \item \textbf{Maze patterns}: Constrained paths along walls limit the corridor advantage, as the optimal path often deviates significantly from the Bresenham line.
    \item \textbf{Room patterns}: Very high effective density (90.9\%) but structured passages favor direct A* search over corridor-based approaches.
\end{itemize}
...
\subsubsection{Density Sensitivity}
Performance is strongly correlated with obstacle density:
\begin{itemize}
    \item \textbf{Low density (10-20\%)}: AILS achieves both time and node improvements
    \item \textbf{Medium density (25-30\%)}: Strong node reduction with moderate time overhead
    \item \textbf{High density (35\%+)}: Performance degrades as corridor expansion dominates
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Localized Adaptive Corridor Mechanism}: We introduced a novel per-point density-based corridor radius computation that adapts to local environmental conditions, achieving search space restriction without preprocessing.

    \item \textbf{Density Estimation Framework}: We developed efficient local obstacle density estimation using integral images and gradient-based lookahead, enabling real-time corridor construction.

    \item \textbf{Multi-Strategy Integration}: We presented a unified framework combining base and density-adaptive strategies with automatic selection based on environmental characteristics.

    \item \textbf{Algorithm-Agnostic Design}: We demonstrated that AILS can enhance any graph search algorithm without requiring algorithm-specific modifications, providing a general-purpose optimization layer.

    \item \textbf{Comprehensive Empirical Validation}: We conducted extensive experiments across grid sizes from 50$\times$50 to 500$\times$500 with densities from 10-40\% and five obstacle patterns. Results demonstrate:
    \begin{itemize}
        \item 51-56\% node reduction vs A* with statistical significance ($p < 0.001$)
        \item Medium-to-large effect sizes (Cohen's $d$ = 0.76--0.82)
        \item Positive scalability with 76.8\% node reduction on 500$\times$500 grids
        \item Time-competitive performance on grids 300$\times$300 and larger
    \end{itemize}
    ...
    \item \textbf{Environment Characterization}: We identified the optimal operating conditions for AILS: random/open obstacle patterns with 10-25\% density on medium-to-large grids, providing practitioners with clear deployment guidance.
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Node Reduction vs Time Trade-off}: AILS achieves substantial node reduction (51-76\%) but corridor construction overhead means time improvements require sufficiently large grids (300$\times$300+). This suggests AILS is best suited for large-scale pathfinding where search space reduction provides significant benefits.

    \item \textbf{Environment Sensitivity}: AILS performance varies significantly with obstacle patterns. Random and open environments show the best results; clustered, maze, and room patterns present challenges due to high local density concentrations or constrained passages.

    \item \textbf{Density Sweet Spot}: The optimal operating range is 10-25\% obstacle density. Below 10\%, standard A* is already efficient; above 30\%, corridor expansion overhead dominates.

    \item \textbf{Scalability Benefits}: AILS's node reduction improvements scale positively with grid size (5.1\% at 50$\times$50 to 76.8\% at 500$\times$500), making it increasingly valuable for larger environments.

    \item \textbf{Practical Deployment}: AILS is most suitable for large-scale navigation in moderately complex environments---applications such as outdoor robotics, warehouse logistics, and open-plan indoor navigation.
\end{itemize}
...
Our comprehensive evaluation identifies both the strengths and limitations of the approach:
\begin{itemize}
    \item \textbf{Strengths}: Significant node reduction, positive scalability, no preprocessing required, algorithm-agnostic design
    \item \textbf{Limitations}: Corridor construction overhead on small grids, performance degradation in high-density and structured (maze/room) environments
\end{itemize}
