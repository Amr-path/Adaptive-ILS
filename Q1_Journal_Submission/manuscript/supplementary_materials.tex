...
If a path exists between start~$s$ and goal~$g$ in grid~$G$, AILS will find one.
\end{theorem}

\begin{proof}
AILS maintains completeness through its incremental expansion mechanism.

\textbf{Base case.} Let $C_a^{(0)}$ be the initial adaptive corridor.
If any valid $s$-$g$ path lies entirely within $C_a^{(0)}$, the constrained search with~$\mathcal{A}$ will find it, because $\mathcal{A}$ is complete on the finite subgraph induced by~$C_a^{(0)}$.

\textbf{Inductive step.} Suppose after $k$ expansion rounds the corridor $C_a^{(k)}$ does not contain any valid path.
The $(k{+}1)$-th expansion adds all traversable cells within Chebyshev distance~$\Delta r$ of the boundary of~$C_a^{(k)}$, so $C_a^{(k)} \subsetneq C_a^{(k+1)}$.

\textbf{Termination.} Because $|C_a^{(k)}|$ strictly increases at each expansion and $|V|$ is finite, the corridor reaches $V$ after at most $O(\max(H, W_g) / \Delta r)$ rounds.
At that point the constrained search is equivalent to an unconstrained search on~$G$, which is complete by assumption.
